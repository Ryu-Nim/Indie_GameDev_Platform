...
@section('content')
    <div class="max-w-7xl w-full mx-auto">
        <div class="mb-6 w-full max-w-7xl">
            <h1 class="text-2xl font-bold text-gray-800 mb-4">Kelola Game</h1>
            @if(session('success'))
                                ...
                                <td class="px-4 py-2 flex gap-2 items-center">
                                    <form action="{{ route('backend.admin.updateGame', $game->id) }}" method="POST"
                                        ...
                                        <select name="status"
                                            ...
                                            <option value="ditinjau" {{ $game->status == 'ditinjau' ? 'selected' : '' }}>Ditinjau
                                            </option>
                                            <option value="tidak_aktif" {{ $game->status == 'tidak_aktif' ? 'selected' : '' }}>
                                                Tidak Aktif</option>
                                            <option value="aktif" {{ $game->status == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                            <option value="ditolak" {{ $game->status == 'ditolak' ? 'selected' : '' }}>Ditolak
                                            </option>
                                            <option value="banned" {{ $game->status == 'banned' ? 'selected' : '' }}>Banned
                                            </option>
                                        </select>
